...
import {NgControl, FormControl, SelectControlValueAccessor} from '@angular/forms'
...
@Component({
  ...
  template: `
    <mat-selection-list cdkDropList [formControl]="formControl" (cdkDropListDropped)="drop($event)">
      <mat-list-option cdkDrag *ngFor="let device of devices$ | async; index as i" [value]="device"> {{ i + 1 }}°. {{ device.label }} </mat-list-option>
    </mat-selection-list>
  `,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DevicesComponent extends SelectControlValueAccessor implements OnInit {
  ...
  drop(event: CdkDragDrop<MediaDeviceInfo[]>) {
    moveItemInArray(this._devices.value, event.previousIndex, event.currentIndex)
    if (!this.formControl.value) this.formControl.setValue([])
    const newValue = this._devices.value.filter((device) => this.formControl.value.includes(device))
