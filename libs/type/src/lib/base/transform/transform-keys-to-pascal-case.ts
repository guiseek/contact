...
type CallRecursiveTransformToPascalCaseIfObject<T> = T extends Record<any, any> ? TransformKeysToPascalCase<T> : T
...
export type TransformKeysToPascalCase<
  ...
  T2 = {
    [K in keyof T1]: CallRecursiveTransformToPascalCaseIfObject<T[Cast<T1[K], string>]>
  }
> = T2
