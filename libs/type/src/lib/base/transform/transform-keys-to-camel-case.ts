...
type CallRecursiveTransformToCamelCaseIfObject<T> = T extends Record<any, any> ? TransformKeysToCamelCase<T> : T
...
export type TransformKeysToCamelCase<
  ...
  T2 = {
    [K in keyof T1]: CallRecursiveTransformToCamelCaseIfObject<T[Cast<T1[K], string>]>
  }
> = T2
